...
$('.test:odd()').css({"backgroundColor":"royalblue", "color":"white"})

// 클래스 선택자 : test를 선택해서 짝수 인덱스 요소만
// 배경색상 : lightpink, 글자색 : white
$('.test:even').css({"backgroundColor":"lightpink", "color":"white"})


// h4 중 클래스 선택자 test가 아닌 요소를 선택해서
//배경색상 : chocolate, 글자색 : white
$('h4:not(.test)').css({"backgroundColor":"chocolate", "color":"white"})
...
// .wrap인 요소의 자식의 자식 요소중 ul태그에 style3을 적용
$('.wrap').children().children('ul').css(style3)

// .wrap인 요소의 자식의 자식 자식 요소 중에 li 태그에 style4를 적용
// $('.wrap').children().find('li').css(style4)
$('.wrap').children().children().children('li').css(style4);

// .wrap인 요소의 모든 하위 요소 중 span 태그의 style5를 적용
$('.wrap *').children('span').css(style5);
...
$('.wrap2 h2').prevUntil('.wrap2 p').css('font-size','2rem');