...
- 선택된 요소의 content 영역(시작 태그 ~ 종료 태그) 리턴하고 변경하는 메소드이다. (HTML 태그를 태그로 인식 inner html과 유사함)

- 클래스 선택자로 여러 요소가 선택되었을 때 첫번째 요소의 텍스트를 리턴하고, 전체 요소의 텍스트 리턴 시 콜백 함수를 사용한다.
...
**속성 수정에는 attr prop input의 밸류값 수정은 var prop은 리턴값이 true false attr은 undifined**
...
- 선택자에 의해 선택된 요소의 스타일 속성 값을 가져오거나 설명하는 메소드
...
- 특정 요소를 선택자로 지정한 요소에 추가하는 메소드 (역할은 같지만 위와 반대)

1. `$('생성구문').appendTo('선택자')` : 특정 요소를 선택자로 지정한 요소의 자식요소로 뒷부분에 추가

2. `$('생성구문').prependTo('선택자')` : 특정 요소를 선택자로 지정한 요소의 자식요소로 앞부분에 추가


3. `$('생성구문').insertAfter('선택자')` : 특정 요소를 선택자로 지정한 요소의 형제요소를 뒷부분에 추가

4. `$('생성구문').insertBfter('선택자')` : 특정 요소를 선택자로 지정한 요소의 형제요소로 앞부분에 추가

## 요소 복제

- 선택자로 선택된 요소를 복사하는 메소드

- 선택된 요소의 이벤트까지 복제할 것인지 지정하는(기본값 : fasle)
```js
...
- bind() 메소드는 제이쿼리 3.0버전부터 사용하지 않기 때문에 on() 메소드를 사용할 것을 권장
...
- **키보드 이벤트** : keyup,  keypress,  keydown,
