...
- **no-repeat** : 배경 이미지를 한 번만 출력하고 반복하지 않는다. <!--자주 사용함-->
...
- **cover** <!--제일 자주 씀--> : 요소를 배경 이미지가 모두 덮을때까지 확대
- 크기 : 너비와 높이 값을 지정
...
- padding-box : 테두리를 뺸 패딩 (padding)이 기준(기본값)
...
- **fixed** : 화면이 스크롤 되더라도 배경 이미지는 고정<!-- attachment는 거의 fixed를 하기위해 씀-->
...
```css
선택자 {
    border-style: none or..... or solid; /*거진 solid만씀*/
}
```
...
- width : 뷰포트의 너비 (device-widht or 크기)
- height : 뷰포트의 높이 (device-height or 크기)
...
- 사이트에 접속하는 장치에 따라 특정한 CSS스타일을 적용시키는 방법

```css
@meida 미디어유형 [and 조건]...  and로 그 외 조건 추가

@media screen and (min-width: 200px) and(max-width: 360px){
    ....
}/*이건 비추천*/

@media screen and(min-widht:700px){
...
## grid-tmplate-rows, grid-template-columns
...
- auto-fill은 셀의 개수가 모자라면 공간이 남지만, auto-fit은 채워버림
